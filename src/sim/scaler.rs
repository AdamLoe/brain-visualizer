...
use crate::sim::backend::{SimConfig, Tier};

/// Per-tier (N, K) ranges (BV18). The scaler may compress/expand within these.
...
impl TierRange {
    pub fn for_tier(tier: Tier) -> Self {
        match tier {
            // Provisional caps (architecture §9.1: browser numbers pending).
            Tier::Low => TierRange {
                n_min: 10_000,
                n_max: 50_000,
                k_min: 16,
...
mod tests {
    ...
    fn over_budget_shrinks() {
        let c = cfg(100_000, Tier::Balanced);
    ...
    fn shrink_clamps_to_tier_min() {
        let c = cfg(50_000, Tier::Balanced);
