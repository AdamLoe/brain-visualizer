...
correct as long as the compared interval is less than half the wrap range.
...
fresh spike — to preserve the silent-start look.
...
S = 2^12 keeps individual synaptic weights in a comfortable range, but
fan-in during synchronised firing can accumulate enough contributions to
overflow i32. Production code must enforce an overflow policy for every
tier — see [`../decisions/data-layout.md`](../decisions/data-layout.md).
