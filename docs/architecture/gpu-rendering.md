...
The tube pass is now a traveling packet rather than whole-arbor instant lighting. `render_morphology.wgsl → vs_main` exports `path_pos = seg.path_len + t * length(seg.b - seg.a)` (`t = 0/1` at the two tube rings, then interpolated per fragment). `fs_main` combines that `path_pos` with the source neuron's packed `last_spike[neuron_id]` to build a source-owned moving impulse: axons carry the full outward packet, dendrites use a much weaker near-soma echo, and a small legacy whole-arbor glow remains only as a background support term. `light_past` is still **removed from the settings surface** (Float32Array index 9 = `reserved_zero`, uniform field always `0`) because upstream lighting on shared arbors remains misleading. See [`../decisions/rendering.md`](../decisions/rendering.md) for rationale.
...
**Opacity model.** `fs_main_active` / `fs_sphere_active` compute `active_alpha = mix(inactive_opacity_floor, active_opacity, clamp(activity, 0, 1))`, where `activity` is the same spike-driven signal the lighting uses (traveling packet for tubes, glow/flash for somas). A freshly-fired fragment writes near-opaque color AND a near-camera depth value; a resting fragment falls to the floor. Fragments below `active_alpha < 0.004` `discard`, so resting/zero-opacity fragments write neither color nor depth (the in-shader inactive skip). `active_opacity` (ceiling, default `1.0`) and `inactive_opacity_floor` (floor, default `0.0` = resting structure fully hidden in the opaque layer, still shown softly by the additive layer) live in the dev-panel-owned `LightingConfig` (`crates/brain-visualizer/src/sim/morphology.rs → LightingConfig`), fed through the two repurposed trailing `MorphUniforms` fields (see Morphology pass) — NOT the `VisualSettings` Float32Array.

**Skip-at-zero.** `crates/brain-visualizer/src/sim/gpu/mod.rs → render_full` computes `active_opaque_on = connection_layer != 0 && active_opacity > 0.0 && <both active pipelines present>`. When `active_opacity == 0` nothing is encoded — not even the depth clear — so the frame is bit-for-bit the pure additive look (mirrors the `draw_surface` / `connection_layer` pass skips).
...
`crates/brain-visualizer/examples/render_check.rs` drives the full production GPU pipeline on llvmpipe (offscreen 512×512), warms the sim for 300 ticks, and asserts pixel-level correctness: not all-black, distinct region colors present, stimulate produces measurable activity increase. It also proves the active-opacity layer: an `active_opacity = 1` frame differs from an `active_opacity = 0` (skipped) frame by more than the llvmpipe-dither epsilon (the active layer drew solid pixels), while the skipped frame matches the additive baseline (the CPU-side pass skip). `crates/brain-visualizer/examples/near_lod_check.rs` exercises the frustum-cull → indirect draw path at close camera distance. Both validate all shaders via Naga at pipeline-build time (panics on any WGSL error).
