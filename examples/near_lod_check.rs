...
//!
//! Verifies that the GPU indirect-draw near-LOD pipeline works correctly:
//!   1. At close camera distance (< 0.8 world units): near-LOD cull + indirect draw runs,
//!      emits > 0 neuron AND synapse instances, renders non-black pixels.
//!   2. At far camera distance (> 1.5 world units): near-LOD is skipped (instance counts
//!      stay 0 / passes not encoded).
...
async fn run() {
    ...
    // Assert frustum cull actually selected geometry.
    assert!(
        close_stats.emitted_neuron_instances > 0,
        "Expected >0 emitted neuron instances at close zoom, got {}",
        close_stats.emitted_neuron_instances
    );
    assert!(
        close_stats.emitted_synapse_instances > 0,
        "Expected >0 emitted synapse instances at close zoom, got {}",
        close_stats.emitted_synapse_instances
    );
