...
fn neuron_color(id: u32) -> vec3<f32> {
    let h = f32((id * 2654435761u) >> 16u) / 65535.0;
...
/// Neurons closer than this to the camera are culled — prevents rendering the
/// interior of spheres the camera is embedded in.
const MIN_CAMERA_DIST: f32 = 0.25;
...
fn cull_neurons(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if distance(pos, frustum.camera_pos) < MIN_CAMERA_DIST { return; }
...
fn cull_synapses(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var j: u32 = 0u; j < cu.k_near; j++) {
        ...
        let ticks_since = tick_diff_local(frustum.current_tick, packed & TICK_MASK);
        let src_glow = select(0.0, exp(-f32(ticks_since) / GLOW_TAU), has_spiked(packed));
        synapse_instances[idx] = SynapseInstance(src_pos, w_sign, tgt_pos, src_glow);
    }
}
