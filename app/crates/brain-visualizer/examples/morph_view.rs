...
async fn run() {
    ...
    for (idx, &(az, el, dist)) in views.iter().enumerate() {
        ...
        println!(
            "[morph_view] view {idx} (az={az:.2} el={el:.2} dist={dist:.2}) → {path}  \
    ...
    // Prove non-active structure is hidden so only live signal pulses show.
    {
        ...
        backend.render_full(
            &color_view, &mvp, camera_right, camera_up,
            100.0, 0.012, camera_pos, dist,
        );
        ...
        let total = WIDTH * HEIGHT;
        let frac = non_black as f32 / total as f32 * 100.0;
        std::fs::write("/tmp/morph_3.rgba", &pixels).expect("write rgba");
        println!(
